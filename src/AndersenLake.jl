...
struct ALCharFuncPricer{T}
    ...
    y::Array{T,1}
    w::Array{T,1}
    function ALCharFuncPricer(
        cf::CharFunc{MT,CR,CF},
        τ::T;
        n::Int = 200,
        tol::T = eps(T),
        qtol::T = sqrt(eps(T))
    ...
    h = qtol
        if n > 0
        h = lambertW(Float64(2 * Base.pi * n)) / n
        for i = -n:n
            q = exp(-Base.pi * sinh(i * h))
            yi = 2 * q / (1 + q)
            y[n+i+1] = yi
            w[n+i+1] = yi * h * Base.pi * cosh(i * h) / (1 + q)
...
function priceEuropean(
    ...
    I = Base.zero(T)
    n = trunc(Int, (length(p.w) - 1) / 2)
    if n == 0
        # c0 = cinf(model(p.cf),τ)
        @inline function integrandLog(x::T)::T
            if x == 1
                return Base.zero(T)
            end
            u = (1 + x) / (1 - x)
        ...
        sqrteps = sqrt(eps(T))
        tol = abs(ω * α) > 20 ? sqrt(sqrteps) : sqrteps
        if abs(ω * α) < logmax - sqrteps
            I = modlob(integrandLog, -one(T), one(T), tol = p.h/forward)
             # I,_ = quadde(integrand, 0, Inf,rtol=1e-8)
        end
    else
        Il = Base.zero(T)
        Iprev = one(T)
...
@inline function kmp(cf::CharFunc{HestonParams{T},CR,CF}, τ::T, x::T)::Tuple{T,T} where {T,CR,CF}
    p = model(cf)
    κ = p.κ
    σ = p.σ
    ρ = p.ρ
...
@inline function evaluateβ(cf::CharFunc{HestonParams{T},CR,CF}, k::T)::T  where {T,CR,CF}
    p = model(cf)
    β = p.κ - p.ρ * p.σ * k
    return β
end

@inline function evaluateD2(cf::CharFunc{HestonParams{T},CR,CF}, β::T, k::T)::T  where {T,CR,CF}
    p = model(cf)
    return D2 = β^2 - p.σ^2 * k * (k - 1)
end

function evaluateK(cf::Union{ CharFunc{HestonParams{T},CR,CF}, CharFunc{SchobelZhuParams{T},CR,CF}}, τ::T, k::T)::T where {T,CR,CF}
    β = evaluateβ(cf, k)
...
@inline function kmp(cf::CharFunc{SchobelZhuParams{T},CR,CF}, τ::T, x::T)::Tuple{T,T} where {T,CR,CF}
    p = model(cf)
    κ = p.κ
    σ = p.σ
    ρ = p.ρ
...
@inline function evaluateβ(cf::CharFunc{SchobelZhuParams{T},CR,CF}, k::T)::T  where {T,CR,CF}
    p = model(cf)
    β = 2*(p.κ - p.ρ * p.σ * k)
    return β
end

@inline function evaluateD2(cf::CharFunc{SchobelZhuParams{T},CR,CF}, β::T, k::T)::T  where {T,CR,CF}
    p = model(cf)
    return D2 = β^2 - 4*p.σ^2 * k * (k - 1)
end

#ω is log(F/K)
function optimalAlpha(cf::Union{ CharFunc{HestonParams{T},CR,CF}, CharFunc{SchobelZhuParams{T},CR,CF}}, τ::T, ω::T)::T where {T,CR,CF}
    ...
    @inline kObjective(x) = evaluateK(cf, τ, x)
    local αmin, αmax
    ismaxOk = true
    if ω < 0
        local kmax
