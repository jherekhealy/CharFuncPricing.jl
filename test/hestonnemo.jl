...
@testset "HestonALSmallPrices" begin
    ...
    τ = R(0.0025)
    strike = R(100)
    spots = [R("100.0001"), R(101.0)]#, R(110.0), R(200.0), R(1000.0), R(10000.0)]
    ...
    prices = Vector{arb}(undef, length(spots))
    for (i, spot) in enumerate(spots)
        prices[i] = priceEuropean(pricer, false, strike, spot, τ, R(1))
        println(spot, " ", prices[i])
    end
    setprecision(512)
    κ = big(2.0)
    ...
    pricer = makeCosCharFuncPricer(cf, τ, m, l)
    pricesB = Vector{BigFloat}(undef, length(spots))
    for (i, spot) in enumerate(spots)
        pricesB[i] = priceEuropean(pricer, false, strike, spot, τ, big(1.0))
        println(spot, " ", pricesB[i])
    end
    @test isapprox(Float64(  prices[1]- R(pricesB[1])), 0.0, atol = 1e-150)
    @test isapprox(Float64( prices[2] - R(pricesB[2])), 0.0, atol = 1e-150)

    #checks that bigfloat implementation of andersen lake is correct (use of pi)
    quad = TanhSinhQuadrature(10000, BigFloat("1e-300"))
    pricer = ALCharFuncPricer(cf,quad)
    pricesC = Vector{BigFloat}(undef, length(spots))
    for (i, spot) in enumerate(spots)
        pricesC[i] = priceEuropean(pricer, false, strike, spot, τ, big(1.0))
        println(spot, " ", pricesC[i])
    end
    @test isapprox(  pricesB[1], pricesC[1], atol = BigFloat(1e-130))
    @test isapprox( pricesB[2] ,pricesC[2], atol = BigFloat(1e-130))
